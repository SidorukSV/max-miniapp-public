...
доверенный TLS-сертификат. Для подключения к `platform-api2.max.ru` скрипт
...
- `url` должен содержать полный корень опубликованного HTTP-сервиса 1С, включая имя базы и `/hs/omni/v1`.
...
Backend добавляет к этому URL пути методов, например `/settings` и `/catalogs/clients`.
...
## Автоматические версии компонентов и сборка CFE

Workflow `.github/workflows/component-release.yml` запускается после обновления `main`, в том числе после merge `dev → main`.
Он независимо вычисляет SHA-256 содержимого каталогов:

- `frontend`;
- `backend`;
- `onec/omni/src/extension/бит_МедицинаОмни_ПРОФ`.

Patch-версия увеличивается только у компонента с изменившейся хеш-суммой. Состояние версий и последних хешей хранится в `.release/components.json`.
Версии frontend/backend записываются в `package.json` и `package-lock.json`. Для расширения увеличивается последняя часть версии в `Configuration.xml`.
При первом запуске workflow исходные хеши еще не заполнены, поэтому версии всех трех компонентов будут увеличены один раз.

При изменении исходников расширения workflow собирает CFE, сохраняет его как Actions artifact и публикует GitHub Release с тегом `onec-v<версия>`.
Версия фиксируется в `main` только после успешной сборки CFE.

Для сборки требуется self-hosted Windows runner с метками `Windows`, `X64`, `onec` и установленной платформой 1С.
В GitHub Actions Variables репозитория необходимо задать:

- `ONEC_V8_PATH` — путь к `1cv8.exe` или каталогу `bin`;
- `ONEC_BASE_CF` — путь на runner к совместимому CF основной конфигурации;
- `ONEC_SEED_CFE` — необязательный путь к исходному CFE, если платформе требуется предварительно создать расширение перед загрузкой XML.

Рекомендуется использовать базовый CF с совместимыми UUID заимствованных объектов. Если исходники расширения были получены из готового CFE, задайте `ONEC_SEED_CFE`: workflow сначала загрузит исходный CFE, затем наложит XML-исходники и соберет новый файл без применения расширения к временной базе.
Workflow использует `contents: write`; правила защиты `main` должны разрешать GitHub Actions записывать служебный commit с версиями.

### Видимость страниц из 1С

При загрузке профиля backend вызывает `GET <url>/settings`. Ожидаемый ответ:
...
- `bonuses: false` скрывает страницу бонусов, вкладку в нижнем меню и карточку бонусов в профиле.
- `survey: false` скрывает анкеты и сообщения на главной, а также блокирует прямые маршруты анкет.
- Если `/settings` недоступен или отдельный флаг отсутствует, для обратной совместимости используется `true`.
...
`VITE_THEME_CONFIG_PATH`, `VITE_PRIVACY_POLICY_URL`, `VITE_PERSONAL_DATA_CONSENT_URL`, `VITE_COMMUNICATION_CONSENT_URL`.

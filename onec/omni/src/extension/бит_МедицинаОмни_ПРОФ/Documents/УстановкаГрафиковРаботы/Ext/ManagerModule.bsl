Функция бит_омни_ДанныеДокументов(HTTPЗапрос) Экспорт
	...
	ИначеЕсли Формат = "DoctorWorkplace" Тогда
		Возврат бит_СменыВрачаНаДень(ИдентификаторСотрудника, ДатаЗаписи);
	//-бит_Сафаров ШИ 31.08.2026
	Иначе
...
Функция бит_СменыВрачаНаДень(ИдентификаторСотрудника, ДатаЗаписи)
	...
	|	ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Графики.Филиал)) КАК branchId,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(Графики.Филиал) КАК branchTitle,
	...
	|	И Графики.ВидВремени В (&ВыгружаемыеВидыВремени)
	|
	|УПОРЯДОЧИТЬ ПО
	|	time_begin";
	...
	ТелоОтвета = ОбщегоНазначения.ТаблицаЗначенийВМассив(Запрос.Выполнить().Выгрузить());
...
Функция СформироватьТаблицыДляОбщегоГрафика(ДатаНачала, ДатаОкончания, Филиал = Неопределено, Сотрудники = Неопределено, 
	...
	Для Каждого стр Из ТаблицаСотрудников Цикл
		...
		Иначе
		//+бит_Сафаров ШИ 31.08.2026
		БазоваяПериодичность = УправлениеНастройками.ПолучитьУчетнуюПолитику(ТекущаяДатаСеанса()).ПериодичностьПланирования;
		//-бит_Сафаров ШИ 31.08.2026
